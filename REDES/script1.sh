...
###############################################
    ...
    aws ec2 modify-vpc-attribute \
        ...
        --enable-dns-hostnames "{\"Value\":true}"
            # Con esto habilitamos la resolución de nombres.
...
####################################################
    ...
    ATTACH_STATE=$(echo $ATTACH_STATE | cut -d',' -f2 | cut -d':' -f3 | cut -d'"' -f2)>/dev/null
        ...
        #########################################################
        ##      IMPRESIÓN ESTADO DE ASOCIACIÓN (OPCIONAL)      ##
        #########################################################
        echo "-------------------------------------------------------------"
    ...
    aws ec2 create-route --route-table-id $RT_ID \
        ...
        --gateway-id $GW_ID
            # Indicamos que todo el tráfico IPv4 vaya por la GateWay.
...
#################################################
####                                         ####
####    CREACIÓN DE EL GRUPO DE SEGURIDAD    ####
####                                         ####
#################################################
